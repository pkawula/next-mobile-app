...
const Home: NextPageWithLayout = () => {
	...
	return (
		...
		>
			...
			{data && data.length > 0 && (
				<div className="flex gap-4 flex-wrap max-w-screen-xl mx-auto my-4 px-2">
					{data.map(({ id, name, picture, price }) => (
						<Link
							href={`/product/${id}`}
							key={id}
							className="flex flex-col gap-1 rounded-md max-w-[320px]"
						>
							...
							<div className="flex gap-2">
								<button
									...
									onClick={(e) => add(e, { id, name, picture, price })}
								>
									Buy now
								</button>
							</div>
